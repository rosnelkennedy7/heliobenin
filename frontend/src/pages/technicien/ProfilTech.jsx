...
const labelSt = { display: 'block', color: 'rgba(255,255,255,0.45)', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.35rem' }
const inputSt = (readOnly) => ({
...
function Field({ label, value, onChange, readOnly }) {
  return (
    <div>
      ...
      <input type="text" value={value} onChange={onChange} readOnly={readOnly} style={inputSt(readOnly)} />
    </div>
  )
}
...
export default function ProfilTech() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundImage: `url(${vitreImg})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, marginLeft: 190, minHeight: '100vh', padding: '2rem', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <div style={{ maxWidth: 520, width: '100%', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ background: 'rgba(15,23,42,0.85)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <Field label="Prénom"  value={form.prenom}  onChange={set('prenom')}  readOnly={false} />
              <Field label="Nom"     value={form.nom}     onChange={set('nom')}     readOnly={false} />
            </div>

            <Field label="Email"      value={form.email}      onChange={set('email')}      readOnly={true}  />
            <Field label="WhatsApp"   value={form.whatsapp}   onChange={set('whatsapp')}   readOnly={false} />
            <Field label="Entreprise" value={form.entreprise} onChange={set('entreprise')} readOnly={false} />
            <Field label="Spécialité" value={form.specialite} onChange={set('specialite')} readOnly={false} />

            {/* IFU + RCCM — 2 colonnes */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <Field label="IFU"  value={form.ifu}  onChange={set('ifu')}  readOnly={false} />
              <Field label="RCCM" value={form.rccm} onChange={set('rccm')} readOnly={false} />
            </div>
          </div>
